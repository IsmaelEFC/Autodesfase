...
// Función para capturar un frame usando requestVideoFrameCallback
function captureFrame(video) {
  return new Promise((resolve, reject) => {
    // Verificar si el navegador soporta requestVideoFrameCallback
    ...
    // Usar requestVideoFrameCallback para asegurar que el frame esté listo
    video.requestVideoFrameCallback(() => {
      try {
        // Crear un canvas con las dimensiones del video
        const canvas = document.createElement('canvas');
        canvas.width = video.videoWidth || 640;
        canvas.height = video.videoHeight || 480;
        ...
        // Devolver la imagen en formato data URL
        resolve(canvas.toDataURL('image/jpeg', 0.9));
      } catch (error) {
        reject(error);
      }
...
async function generarCaptura() {
  ...
  try {
    ...
    console.log('Dimensiones del video:', camera.videoWidth, 'x', camera.videoHeight);
    
    try {
      // Capturar el frame usando requestVideoFrameCallback
      capturedFrame = await captureFrame(camera);
      ...
      throw new Error(`Error al capturar el frame: ${error.message}`);
    }
